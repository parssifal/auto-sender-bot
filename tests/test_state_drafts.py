...
@pytest.mark.parametrize(
    "table, expected_columns, expected_fk_tables, expected_indexes",
    [
        (
            "teams",
            EXPECTED_TEAM_COLUMNS,
            {"users"},
            {"sqlite_autoindex_teams_1", "idx_teams_owner_created"},
        ),
        (
            "team_members",
            EXPECTED_TEAM_MEMBER_COLUMNS,
...
async def test_state_store_migrate_creates_schema(
    store, table, expected_columns, expected_fk_tables, expected_indexes
) -> None:
    conn = store._conn
    ...
    foreign_keys = await conn.execute_fetchall(f"PRAGMA foreign_key_list({table})")
    assert {str(foreign_key["table"]) for foreign_key in foreign_keys} == expected_fk_tables

    if expected_indexes is not None:
        indexes = await conn.execute_fetchall(f"PRAGMA index_list({table})")
        assert {str(index["name"]) for index in indexes} >= expected_indexes


@pytest.mark.asyncio
async def test_state_store_migrate_is_idempotent_for_teams_and_backfills_owner_membership(store) -> None:
...
@pytest.mark.asyncio
async def test_state_store_migrate_is_idempotent_for_drafts(store) -> None:
    await store.ensure_user(123)
...
@pytest.mark.asyncio
async def test_state_store_team_members_enforce_single_owner(store) -> None:
    await store.ensure_user(123)
    await store.ensure_user(456)
...
@pytest.mark.asyncio
async def test_state_store_team_invite_accepts_once_and_adds_membership(store, monkeypatch: pytest.MonkeyPatch) -> None:
    await store.ensure_user(123)
    await store.ensure_user(456)
...
@pytest.mark.asyncio
async def test_state_store_team_invite_rejects_non_owner_expired_and_existing_members(store, monkeypatch: pytest.MonkeyPatch) -> None:
    await store.ensure_user(123)
    await store.ensure_user(456)
    await store.ensure_user(789)
